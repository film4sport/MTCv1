...
- **Android Booking Fixes:** Resolved "Please log in" error by migrating to the unified `mtc-user` storage object. Fixed a timezone mismatch that rejected valid evening bookings by enforcing the `America/Toronto` timezone in the API. Corrected an AM/PM 12-hour offset in the booking modal UI.
- **Announcement Inbox Spam:** Altered announcement broadcasting to be sent from a system `'club'` ID instead of the admin's personal ID, preventing the admin's inbox from being flooded with 1-on-1 chats to every member.
- **Push Notification Metadata:** Added `conversationMetaMap` to `localStorage` sync so waking the app from a push notification immediately shows the sender's name/avatar instead of briefly flashing their raw UUID.
- **Home Calendar UX:** Implemented auto-scroll logic in `home-calendar.ts` that triggers when a date is tapped. The screen now smoothly scrolls to reveal the events list below the calendar, ensuring the user sees the content immediately without manual scrolling.
