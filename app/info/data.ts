...
export const waiverText = `MONO TENNIS CLUB — WAIVER AND RELEASE OF LIABILITY, ASSUMPTION OF RISK AND INDEMNITY AGREEMENT
...
export const membershipTypes = [
  ...
  { key: 'family', label: 'Family', price: 240, desc: 'Up to 2 adults + 4 juniors, one account with switchable profiles' },
  { key: 'junior', label: 'Junior (18 & under)', price: 55, desc: 'Full season access for one junior player' },
  { key: 'guest', label: 'Guest Pass', price: 10, desc: 'Single visit access' },
];

// Signup flow excludes Guest Pass (guests don't need accounts)
export const signupMembershipTypes = membershipTypes.filter(m => m.key !== 'guest');
