...
export async function GET(request: Request) {
  ...
  try {
    ...
    const { data: members, error } = await supabase
      .from('profiles')
      .select('id, name, email, role, ntrp, skill_level, skill_level_set, avatar, member_since, status, preferences, membership_type, family_id, residence, interclub_team, interclub_captain')
    ...
    const result = visibleMembers.map(m => ({
      ...
      interclubTeam: m.interclub_team,
      interclubCaptain: !!m.interclub_captain,
...
/** Add a new member (admin only) â€” creates Supabase auth user + profile */
...
/** Update a member profile â€” members can update own profile, admins can update anyone */
export async function PATCH(request: Request) {
  ...
  try {
    ...
    const memberId = body.memberId || authResult.id;
...
/** Remove a member (admin only) â€” calls delete_member RPC */
