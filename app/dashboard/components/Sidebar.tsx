...
const navItems = [
  { href: APP_ROUTES.dashboard, label: APP_COPY.home, icon: 'M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6' },
  { href: APP_ROUTES.dashboardBook, label: APP_COPY.bookCourt, icon: 'M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z' },
  { href: APP_ROUTES.dashboardSchedule, label: APP_COPY.schedule, icon: 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z' },
  { href: APP_ROUTES.dashboardPartners, label: APP_COPY.partners, icon: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z' },
  { href: APP_ROUTES.dashboardEvents, label: APP_COPY.clubEvents, icon: 'M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z' },
  { href: APP_ROUTES.dashboardLessons, label: APP_COPY.lessons, icon: 'M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253' },
  { href: APP_ROUTES.dashboardMessages, label: APP_COPY.messages, icon: 'M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z' },
  { href: APP_ROUTES.dashboardSettings, label: APP_COPY.settings, icon: 'M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.066 2.573c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.573 1.066c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.066-2.573c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z M15 12a3 3 0 11-6 0 3 3 0 016 0z' },
];

const adminItem = { href: APP_ROUTES.dashboardAdmin, label: APP_COPY.adminPanel, icon: 'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z' };
...
function Sidebar() {
  ...
  const filteredNavItems = useMemo(() => navItems.filter(item => {
    if (isCoach && item.href === APP_ROUTES.dashboardPartners) return false;
    return true;
  }), [isCoach]);
          ...
          <ul className="space-y-1 relative">
            ...
            {(() => {
              const activeIdx = filteredNavItems.findIndex(item =>
                item.href === APP_ROUTES.dashboard ? pathname === APP_ROUTES.dashboard : pathname.startsWith(item.href)
              );
              ...
              return (
                <li key={item.href}>
                  <Link
                    ...
                    data-tour={item.href === APP_ROUTES.dashboardMessages ? 'messages' : item.href === APP_ROUTES.dashboardSettings ? 'settings' : undefined}
            ...
            {isOnTeam && (
              <li className="pt-2 mt-2" style={{ borderTop: '1px solid rgba(232, 228, 217, 0.1)' }}>
                <Link
                  href={APP_ROUTES.dashboardCaptain}
                  onClick={() => handleNavClick(APP_ROUTES.dashboardCaptain)}
                  aria-current={pathname.startsWith(APP_ROUTES.dashboardCaptain) ? 'page' : undefined}
                  className={`sidebar-nav-link flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200 ${
                    pathname.startsWith(APP_ROUTES.dashboardCaptain)
                      ? 'sidebar-nav-active font-semibold'
                      : ''
                  } ${tappedHref === APP_ROUTES.dashboardCaptain ? 'sidebar-tapped' : ''}`}
                  title={sidebarCollapsed ? APP_COPY.myTeam : undefined}
                >
                  <svg className="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={pathname.startsWith(APP_ROUTES.dashboardCaptain) ? 2.5 : 2} aria-hidden="true">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" />
                  </svg>
                  {!sidebarCollapsed && <span className="text-sm">{APP_COPY.myTeam}</span>}
                </Link>
              </li>
            )}
