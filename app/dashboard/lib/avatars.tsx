// Avatar SVGs ported from mobile PWA (public/mobile-app/js/avatar.js)
// 4 tennis player illustrations used as default profile pictures

export const AVATAR_SVGS: Record<string, string> = {
  // ── Tennis Player illustrations (original 4) ──
  ...
  // ── Simple profile avatars (circle head + shirt, matching login mockups) ──
...
export const AVATAR_OPTIONS = [
  { id: 'tennis-male-1', label: 'Player 1' },
  { id: 'tennis-male-2', label: 'Player 2' },
  { id: 'tennis-female-1', label: 'Player 3' },
  { id: 'tennis-female-2', label: 'Player 4' },
  { id: 'simple-1', label: 'Avatar 1' },
  { id: 'simple-2', label: 'Avatar 2' },
  { id: 'simple-3', label: 'Avatar 3' },
  { id: 'simple-4', label: 'Avatar 4' },
  { id: 'simple-5', label: 'Avatar 5' },
  { id: 'simple-6', label: 'Avatar 6' },
  { id: 'simple-7', label: 'Avatar 7' },
  { id: 'simple-8', label: 'Avatar 8' },
] as const;

/** Returns true if the avatar string is a preset SVG ID (not a URL) */
export function isPresetAvatar(avatar: string | undefined): boolean {
  return !!avatar && avatar in AVATAR_SVGS;
}

/** Renders an avatar — either a preset SVG, a URL image, or initials fallback */
export function AvatarDisplay({ avatar, name, size = 40, className = '' }: {
  avatar?: string;
  name: string;
  size?: number;
  className?: string;
  ...
  if (avatar && isPresetAvatar(avatar)) {
    return (
      <div
        ...
        dangerouslySetInnerHTML={{ __html: AVATAR_SVGS[avatar] }}
      />
    );
  }
  ...
  return (
    <div
      ...
      style={{
        width: size,
        height: size,
        background: 'rgba(107, 122, 61, 0.1)',
        color: '#6b7a3d',
        fontSize: size * 0.35,
      }}
    >
